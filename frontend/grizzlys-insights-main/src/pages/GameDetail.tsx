...
// Mock game data (in real app, fetch by ID)
const gameData = {
  id: 1,
  date: "2025-11-22",
...
export default function GameDetail() {
  ...
  const adjustment =
    (teamForm[0] - 50) * 10 + (weather[0] - 50) * 5 - priceChange[0] * 8 + (marketing[0] - 50) * 6;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <Card className="border-border">
          <CardContent className="pt-6">
            ...
            <p className="text-sm text-muted-foreground mb-2">Predicted Tickets</p>
            <div className="text-xs text-muted-foreground">
              Range: {gameData.percentile5.toLocaleString()} - {gameData.percentile95.toLocaleString()}
            </div>
          </CardContent>
        </Card>
        ...
        <Card className="border-border">
          <CardContent className="pt-6">
            ...
            <div className="text-4xl font-bold text-foreground mb-1">{gameData.occupancy}%</div>
      ...
      <Card className="border-border">
        <CardHeader>
          <CardTitle className="text-xl text-foreground">Scenario Simulation</CardTitle>
        ...
        <CardContent className="space-y-8">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium text-foreground">Team Form</label>
                <span className="text-sm text-muted-foreground">{teamForm[0]}%</span>
              </div>
              <Slider value={teamForm} onValueChange={setTeamForm} max={100} step={1} />
            </div>
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <span className="text-sm text-muted-foreground">
                  {weather[0] < 40 ? "Rainy" : weather[0] < 60 ? "Cloudy" : "Clear"}
                </span>
              </div>
              <Slider value={weather} onValueChange={setWeather} max={100} step={1} />
            </div>

            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium text-foreground">Ticket Price Change</label>
                <span className="text-sm text-muted-foreground">{priceChange[0] > 0 ? "+" : ""}{priceChange[0]}%</span>
              </div>
              <Slider value={priceChange} onValueChange={setPriceChange} min={-20} max={20} step={1} />
            </div>

            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <label className="text-sm font-medium text-foreground">Marketing Boost</label>
                <span className="text-sm text-muted-foreground">{marketing[0]}%</span>
              </div>
              <Slider value={marketing} onValueChange={setMarketing} max={100} step={1} />
            </div>
          </div>
          ...
          <Card className="bg-muted border-primary/50">
            <CardHeader>
              <CardTitle className="text-lg text-foreground">Updated Forecast</CardTitle>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-3 gap-4">
                ...
                <div>
                  ...
                  <p className="text-2xl font-bold text-foreground">{adjustedOccupancy}%</p>
                </div>
              </div>
            </CardContent>
          </Card>
      ...
      <Card className="border-border">
        <CardHeader>
          <CardTitle className="text-xl text-foreground">Why This Prediction?</CardTitle>
          <p className="text-sm text-muted-foreground">
